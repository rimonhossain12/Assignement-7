...
const loadProducts = () => {
  // console.log('load data is click');
  // api 
...
const showProducts = (products) => {
  ...
  for (const product of allProducts) {
    ...
    <img class="product-image" src=${image}></img>
      ...
      <h5 class="rating">Total Rating: ${product.rating.count}</h5>
      <button onclick="addToCart(${product.id},${product.price})" id="addToCart-btn" class="buy-now btn btn-success btn-style">add to cart</button>
      <button id="details-btn" class="btn btn-danger btn-style">Details</button></div>

      `;
    document.getElementById("all-products").appendChild(div);
  }
};
// counting the clickable prouducts
let count = 0;
